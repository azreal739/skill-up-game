...
export const apiMission002HttpStatus: MissionDefinition = {
  ...
  summary:
    'The client treats every non-200 the same. Read the status codes and respond correctly.',
  difficulty: 'easy',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      tags: ['api'],
      storyContext:
        'Different status classes call for different handling — retry, re-auth, fix the request, or show the resource is gone.',
      prompt: 'Which response is the right reading of these four failures?',
      options: [
        {
          id: 'b',
          label:
            'Retry all of them a few times — most transient errors clear up',
          isCorrect: false,
          feedback:
            'Retrying a 400 or 401 just repeats a request the server has already rejected for a reason. Only 5xx/timeouts are retry candidates.',
        },
        {
          id: 'a',
          label:
        ...
        },
        {
          id: 'c',
          label:
            'Show the same generic error for all four; the user cannot act on status codes anyway',
          isCorrect: false,
          feedback:
            'The user cannot read the code, but your handling must differ: re-auth on 401, a validation message on 400, and "not found" on 404 are all different UX.',
        },
      ],
      hints: [
        {
          ...
          title: 'Direction',
          content:
            'Group the codes: which are caller problems and which are server problems?',
        },
        {
          level: 2,
          title: 'Concept',
        ...
        },
        {
          ...
          title: 'Specific clue',
          content:
            'Only one of these four (503) is a server problem worth retrying.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content:
            'Fix 400 client-side, re-auth 401, "not found" for 404, retry the 503.',
        },
      ],
      ...
      consequences: [
        {
          ...
          delta: -5,
          reason:
            'Collapsing every failure into one error hid real, actionable differences.',
        },
      ],
      helpLinks: [{ topicId: 'api.status-codes', label: 'HTTP status codes' }],
      successFeedback:
        'Each status now drives the right behaviour — fix, re-auth, not-found, or a bounded retry.',
      failureFeedback:
        '4xx are caller problems (do not retry); 5xx are server problems (retry may help). Only the 503 should be retried here.',
    },
  ],
  reflectionPrompt:
    'Which is more dangerous: retrying a request the server already rejected, or never retrying one it might have served?',
