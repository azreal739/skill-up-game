...
export const apiMission003ErrorPayloadDrift: MissionDefinition = {
  ...
  summary:
    'A failed request crashes a second time on the error path. The error body changed shape.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      tags: ['api'],
      storyContext:
        'The error path assumed a shape that drifted. It needs to read the new shape defensively.',
      prompt: 'What is the right fix for the error handler?',
      options: [
        {
          id: 'b',
          label:
            'Wrap the toast in another try/catch so the second crash is swallowed',
          isCorrect: false,
          feedback:
            'That hides the symptom and shows the user nothing. The error message — the whole point — is lost, and the drift stays invisible.',
        },
        {
          id: 'c',
          label:
            'Change the UI back to reading body.message and ask the back end to revert',
          isCorrect: false,
          feedback:
            'You cannot freeze the back end. The client must read the current contract defensively, not depend on an old one being restored.',
        },
        {
          id: 'a',
          label:
      ...
      hints: [
        {
          ...
          title: 'Direction',
          content:
            'The error body is untrusted input like any response — what do we do with untrusted input?',
        },
        {
          level: 2,
          title: 'Concept',
        ...
        },
        {
          level: 3,
          title: 'Specific clue',
          content: 'Parse with a fallback message, then read error.detail.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content:
            'Schema-validate the error body with a default, and surface error.detail.',
        },
      ],
      ...
      consequences: [
        {
          type: 'stability',
          delta: -10,
          reason: 'An unvalidated error path turned one failure into two.',
        },
      ],
      helpLinks: [
        { topicId: 'api.error-payloads', label: 'Error payloads' },
        {
          topicId: 'zod.runtime-validation',
          label: 'Runtime validation with Zod',
        },
      ],
      successFeedback:
        'The error path now validates its own input and falls back safely — one failure stays one failure.',
      failureFeedback:
        'Swallowing or reverting does not fix drift. Validate the error body with a fallback and read the field that exists now.',
    },
  ],
  reflectionPrompt:
    'Why is the error path more dangerous to leave unvalidated than the happy path?',
