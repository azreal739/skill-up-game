...
export const apiMission004JavaAssumption: MissionDefinition = {
  ...
  summary:
    'Compare the front-end model against the Java DTO and find where they disagree.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      tags: ['api', 'java'],
      storyContext:
        'The DTO is the source of truth. Every field where the front-end model disagrees is a runtime bug waiting to happen.',
      prompt:
        'Select every field where the front-end model disagrees with the Java DTO.',
      options: [
        {
          id: 'amount',
          label:
            'amount — model says number, DTO serialises a string ("49.99")',
          isCorrect: true,
          feedback:
            'A numeric string is not a number until you coerce it. Arithmetic on "49.99" concatenates or yields NaN.',
        },
        {
          id: 'refunded',
        ...
        },
        {
          id: 'receiptUrl',
          label:
            'receiptUrl — model says non-null string, DTO marks it nullable',
          isCorrect: true,
          feedback:
            'A nullable field read as a guaranteed string crashes the moment a payment has no receipt yet.',
        },
        {
          id: 'currency',
        ...
        },
        {
          id: 'createdAt',
          label:
            'createdAt — model expects an ISO string, DTO sends epoch millis (a number)',
          isCorrect: true,
          feedback:
            'new Date("1751846400000") is Invalid Date; new Date(1751846400000) is correct. The type and meaning both differ.',
        },
      ],
      hints: [
        {
          ...
          title: 'Direction',
          content:
            'Go field by field: same name, but same type and nullability?',
        },
        {
          level: 2,
          title: 'Concept',
        ...
        },
        {
          level: 3,
          title: 'Specific clue',
          content: 'Three fields disagree; currency and refunded match.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content:
            'Flag amount (string vs number), createdAt (millis vs ISO), and receiptUrl (nullable).',
        },
      ],
      ...
      consequences: [
        {
          ...
          delta: 10,
          reason:
            'A model built from assumptions rather than the DTO carried three latent bugs.',
        },
      ],
      helpLinks: [
      ...
      successFeedback:
        'You found all three real disagreements — amount, createdAt and receiptUrl — and left the matching fields alone.',
      failureFeedback:
        'Three fields disagree by type or nullability (amount, createdAt, receiptUrl); currency and refunded already match.',
    },
  ],
  reflectionPrompt:
    'How much of contract drift comes down to reading a field’s name and skipping its type?',
