...
export const apiMission006ContractReview: MissionDefinition = {
  ...
  summary:
    'Review the boundary adapter and its schema before it merges. Find what still lets drift through.',
  difficulty: 'hard',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      tags: ['api'],
      storyContext:
        'The adapter is meant to hand the app a clean model and to fail loudly and safely when the contract drifts.',
      prompt: 'Select every genuine problem with this adapter and schema.',
      findings: [
        {
          id: 'receipturl-nullable',
          label:
            'receiptUrl is z.string() with no .nullable(), so a null receipt throws on a perfectly normal payment',
          isCorrect: true,
          feedback:
        ...
        },
        {
          id: 'style-naming',
          label:
            'The schema constant should be named PaymentSchema, not PaymentDtoSchema',
          isCorrect: false,
          feedback:
            'A naming preference with no effect on correctness — DtoSchema is arguably clearer since it describes the wire shape.',
        },
        {
          id: 'parse-throws',
          label:
            'parse() throws on mismatch instead of safeParse with a fallback, so one drifted field takes the whole screen down',
          isCorrect: true,
          feedback:
        ...
        },
        {
          ...
          label:
            'amount is validated as a string but never coerced to a number, so the app still receives "49.99"',
          isCorrect: true,
          feedback:
      ...
      hints: [
        {
          ...
          title: 'Direction',
          content:
            'Does the adapter actually produce the clean model, and does it fail safely?',
        },
        {
          level: 2,
          title: 'Concept',
        ...
        },
        {
          ...
          title: 'Specific clue',
          content:
            'Three real problems: amount not coerced, receiptUrl not nullable, and parse() throwing.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content:
            'Flag the un-coerced amount, the missing .nullable() on receiptUrl, and the throwing parse(). Ignore the two style nits.',
        },
      ],
      ...
      consequences: [
        {
          ...
          delta: 15,
          reason:
            'An adapter that neither cleaned nor tolerated the real contract would have shipped three bugs.',
        },
      ],
      helpLinks: [
      ...
      successFeedback:
        'You caught the three real defects and left the preferences alone — the adapter will clean data and fail safely.',
      failureFeedback:
        'Focus on correctness: amount isn’t coerced, receiptUrl isn’t nullable, and parse() throws into the UI. The naming and enum points are preferences.',
    },
  ],
  reflectionPrompt:
    'In a review, how do you separate "this is wrong" from "this is how I’d have written it"?',
