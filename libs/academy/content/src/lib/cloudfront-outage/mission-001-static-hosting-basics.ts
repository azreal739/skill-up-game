...
export const cdnMission001StaticHosting: MissionDefinition = {
  ...
  summary: 'Users report a stale app after a deploy. First, understand how the app is actually served.',
  difficulty: 'easy',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      difficulty: 'easy',
      tags: ['cicd'],
      storyContext: 'A new engineer suggests SSHing in to restart the server. There is no server running your code.',
      prompt: 'For a static SPA on a CDN, what is the correct mental model of a deploy?',
      options: [
        {
          id: 'a',
          label: 'The build produces static files uploaded to storage and served through a CDN; users run the bundle in their browser, and the CDN sits between deploy and user',
          isCorrect: true,
          feedback:
        ...
        {
          id: 'b',
          label: 'A server runs the Angular app and renders each page on request; restart it to pick up the new code',
          isCorrect: false,
          feedback: 'That describes SSR/a running server. This app is static files — there is nothing to restart, and "restarting" would not touch the CDN cache.',
        },
        {
          id: 'c',
          label: 'The browser always fetches the latest files, so a deploy is instant and caching is irrelevant',
          isCorrect: false,
          feedback: 'The opposite — the CDN and browser cache aggressively. Caching is the whole reason a deploy can look "not live".',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'What actually executes your Angular code — a server, or the user’s browser?' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        { level: 3, title: 'Specific clue', content: 'The CDN between deploy and user is what to think about, not a server restart.' },
  ...
  reflectionPrompt: 'How many "it works on my machine but not in prod" problems are really a cache between you and the user?',
