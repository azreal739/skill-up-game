...
export const cdnMission003CacheBehaviour: MissionDefinition = {
  ...
  summary: 'Review the CDN cache policy. One header is why the last three deploys took hours to appear.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      difficulty: 'medium',
      tags: ['cicd'],
      storyContext: 'index.html and config.json keep the same URL across deploys, so their TTL is exactly how long users wait for a change.',
      prompt: 'Why do deploys take three hours to appear, and what is the fix?',
      options: [
        {
          id: 'a',
          label: 'index.html (and config.json) are cached for 3 hours; since their URLs never change, users keep the old copy that long. Serve them no-cache / very short TTL',
          isCorrect: true,
          feedback:
        ...
        {
          id: 'b',
          label: 'The hashed JS/CSS are cached for a year — shorten those to 3 hours to make deploys faster',
          isCorrect: false,
          feedback: 'The hashed assets are fine: their URLs change per deploy, so a year is correct and efficient. Shortening them just wastes bandwidth and does not fix the delay.',
        },
        {
          id: 'c',
          label: 'CloudFront is simply slow to propagate; nothing in the config is wrong',
          isCorrect: false,
          feedback: 'Propagation is near-instant. The three hours matches the index.html TTL exactly — that is the cause, not propagation.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'The delay is three hours. Which TTL is three hours?' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        { level: 3, title: 'Specific clue', content: 'The 3-hour TTL is on the entry files that reuse their URLs.' },
  ...
  reflectionPrompt: 'For a file whose URL never changes, what does its cache TTL really control?',
