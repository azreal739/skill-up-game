...
export const cdnMission004Invalidation: MissionDefinition = {
  ...
  title: 'Invalidation',
  summary: 'A hotfix needs to reach users now, before the cache expires. Invalidate the right paths.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      difficulty: 'medium',
      tags: ['cicd'],
      storyContext: 'Hashed assets got new URLs and were never cached under those names; only the reused URLs are stale.',
      prompt: 'What should you invalidate?',
      options: [
        {
          id: 'a',
          label: 'Invalidate /index.html (and any other fixed-URL entry files like /config.json) — the new hashed assets need no invalidation',
          isCorrect: true,
          feedback:
        ...
        },
        {
          ...
          isCorrect: false,
          feedback: 'A blanket /* invalidation is slow, can hit invalidation limits/cost, and forces every edge to re-fetch unchanged immutable assets for no benefit.',
        },
        {
          id: 'c',
          label: 'Invalidate the hashed JS/CSS by their new names',
          isCorrect: false,
          feedback: 'Those new names were never cached — there is nothing to invalidate. It is the reused-URL entry files that are stale.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'Which files could still be stale — the ones with new names, or the ones with the same name?' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        { level: 3, title: 'Specific clue', content: 'Invalidate the fixed-URL entry files, not the hashed assets and not /*.' },
  ...
  reflectionPrompt: 'If your assets are properly hashed, how often should you ever need to invalidate anything but index.html?',
