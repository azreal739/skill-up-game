...
export const cdnMission006EnvironmentConfig: MissionDefinition = {
  ...
  summary:
    'Production is calling the staging API. Find how the wrong config reached the live bundle.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      tags: ['cicd'],
      storyContext:
        'A single artifact is promoted across environments, but the API URL was baked in at build time for staging.',
      prompt: 'What is the right fix?',
      options: [
        {
          id: 'b',
          label:
            'Keep baking the URL in, but build a separate production artifact and stop promoting',
          isCorrect: false,
          feedback:
        ...
        },
        {
          id: 'a',
          label:
            'Move the environment-specific API URL out of the bundle into a runtime config (a small config.json the app fetches per environment), so one artifact reads the right URL wherever it runs',
          isCorrect: true,
          feedback:
        ...
        },
        {
          id: 'c',
          label:
            'Hard-code both URLs in the app and switch on window.location.hostname',
          isCorrect: false,
          feedback:
            'Fragile and leaky — every new environment edits the app, and internal hostnames end up shipped in the bundle. Externalise the config instead.',
        },
      ],
      hints: [
        {
          ...
          title: 'Direction',
          content:
            'The wrong URL was fixed at build time. Where should an environment value be decided instead?',
        },
        {
          level: 2,
          title: 'Concept',
        ...
        },
        {
          ...
          title: 'Specific clue',
          content:
            'A per-environment runtime config the app fetches on startup.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content:
            'Externalise API_URL into a runtime config.json resolved per environment.',
        },
      ],
      ...
      consequences: [
        {
          ...
          delta: 1,
          reason:
            'A baked-in staging URL sent production traffic to the wrong backend.',
        },
      ],
      helpLinks: [
        {
      ...
      successFeedback:
        'One artifact, environment resolved at runtime — production talks to production, staging to staging.',
      failureFeedback:
        'If you promote one artifact, decide environment values at runtime, not baked in for staging.',
    },
  ],
  reflectionPrompt:
    'If the same build runs in staging and production, what must be true about anything environment-specific inside it?',
