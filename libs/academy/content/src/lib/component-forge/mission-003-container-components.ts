...
export const cfMission003Container: MissionDefinition = {
  ...
  summary: 'Now build the smart half: the container that feeds the reusable tile.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      difficulty: 'medium',
      tags: ['angular'],
      storyContext: 'The dashboard container has customers$ from the service and an openCustomer() handler.',
      prompt: 'Which container template correctly uses the presentational tile?',
      options: [
        {
          id: 'a',
          label:
        ...
        },
        {
          ...
          isCorrect: false,
          feedback: 'That pushes data-loading back into the presentational tile — exactly the anti-pattern we fixed.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'The container owns the data; the tile just renders one customer.' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        },
        {
          level: 3,
          title: 'Specific clue',
          content: 'Look for the option that binds [customer] and listens to (selected).',
        },
        {
          level: 4,
          title: 'Guided solution',
          content: 'Pick the @for that passes [customer]="c" and handles (selected)="openCustomer($event)".',
        },
      ],
      rewards: [{ type: 'xp', amount: 25, label: 'Container wired' }],
      consequences: [
        { type: 'stability', delta: -5, reason: 'A misused tile rendered blank rows on the dashboard.' },
      ],
      helpLinks: [
      ...
      successFeedback: 'Data down, events up — the container feeds a dumb, reusable tile.',
      failureFeedback: 'The tile takes a resolved customer and emits selection; the container owns everything else.',
    },
  ],
  reflectionPrompt: 'What belongs in a container that should never leak into a presentational child?',
