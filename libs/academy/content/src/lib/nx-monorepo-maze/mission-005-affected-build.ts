...
export const nxMission005Affected: MissionDefinition = {
  ...
  summary:
    'CI rebuilds the whole repo on every PR. Make it build only what changed.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      tags: ['nx', 'cicd'],
      storyContext:
        'A change to one library can only break that library and things that depend on it.',
      prompt: 'How should CI scope its work safely?',
      options: [
        {
          id: 'b',
          label:
            'Only build the exact projects whose files changed, ignore dependents',
          isCorrect: false,
          feedback:
        ...
        },
        {
          ...
          isCorrect: false,
          feedback:
            'Throwing hardware at a whole-repo build is expensive and still wasteful — most projects were unaffected.',
        },
        {
          id: 'a',
          label:
            'nx affected -t build test lint — build/test only the changed projects and everything downstream of them',
          isCorrect: true,
          feedback:
      ...
      hints: [
        {
          ...
          title: 'Direction',
          content:
            'What can a change actually break? Build that, and only that.',
        },
        {
          level: 2,
          title: 'Concept',
        ...
        },
        {
          ...
          title: 'Specific clue',
          content:
            'The safe option includes downstream dependents, not just changed files.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content: 'Use nx affected for build, test and lint.',
        },
      ],
      ...
      consequences: [
        {
          ...
          delta: -5,
          reason: 'A 40-minute pipeline slowed every merge.',
        },
      ],
      helpLinks: [
        {
      ...
      successFeedback:
        'CI now runs only what a change can affect — fast and still safe.',
      failureFeedback:
        'Ignoring dependents is unsafe; more machines is wasteful. affected is the scoped, safe answer.',
    },
  ],
  reflectionPrompt:
    'Why is "changed files only" unsafe, but "changed plus downstream" safe?',
