...
export const prodMission003LogsAndMetrics: MissionDefinition = {
  ...
  summary:
    'Let the data locate the failure. Correlate the spike with what changed.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      storyContext:
        'The error onset is one minute after a deploy, DB/cache/traffic are all normal, and the logs name a v4.7 code path.',
      prompt: 'What does the data point to?',
      options: [
        {
          id: 'b',
          label:
            'The database, since checkout writes to it — restart the DB to be safe',
          isCorrect: false,
          feedback:
            'DB latency is normal and nothing points at it. Restarting an healthy DB is a guess that risks a second problem while ignoring the deploy the data implicates.',
        },
        {
          id: 'a',
          label:
            'The v4.7 checkout-service deploy at 14:01: the error started at 14:02, DB/cache/traffic are normal, and logs show a TypeError in the v4.7 handler',
          isCorrect: true,
          feedback:
        ...
        },
        {
          ...
          isCorrect: false,
          feedback:
            'Traffic is normal for the hour. Scaling up would burn time and money without touching the actual cause in v4.7.',
        },
      ],
      hints: [
        {
          ...
          title: 'Direction',
          content:
            'When exactly did it start, and what changed at that moment?',
        },
        {
          level: 2,
          title: 'Concept',
        ...
        },
        {
          ...
          title: 'Specific clue',
          content:
            'Onset 14:02, deploy 14:01, DB/cache/traffic normal, TypeError in v4.7.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content: 'The v4.7 deploy is the cause; the data converges on it.',
        },
      ],
      ...
      consequences: [
        {
          ...
          delta: 1,
          reason:
            'Guess-driven debugging burned time while checkouts kept failing.',
        },
      ],
      helpLinks: [
        {
      ...
      successFeedback:
        'The data converges on the v4.7 deploy — you have a cause, not a hunch. Now choose how to recover.',
      failureFeedback:
        'DB, cache and traffic are all normal. Onset right after the v4.7 deploy plus a matching stack trace names the cause.',
    },
  ],
  reflectionPrompt:
    'How often is the fastest diagnosis just "what changed right before it started?"',
