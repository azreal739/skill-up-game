...
export const prodMission007ValidateRecovery: MissionDefinition = {
  ...
  summary: 'It looks fixed. Prove it with data before declaring the incident over.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      storyContext: 'Recovery is validated by real, sustained signals across users — not one passing manual test.',
      prompt: 'What justifies declaring the incident recovered?',
      options: [
        {
          id: 'a',
          label: 'Error rate back to baseline and stable for a sustained window, correct sampled totals, and support tickets stopped — real user signals agreeing, not just one manual test',
          isCorrect: true,
          feedback:
        ...
        {
          id: 'b',
          label: 'Your single manual checkout passed — that is enough to close it',
          isCorrect: false,
          feedback: 'One manual test can pass while a cohort still fails. Confirm with aggregate, sustained user metrics before standing down.',
        },
        {
          id: 'c',
          label: 'The error rate dipped for one minute right after the change — declare it resolved immediately',
          isCorrect: false,
          feedback: 'A one-minute dip can be noise. Recovery needs to hold for a sustained window before you trust it.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'What proves real users recovered, not just your one test?' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        { level: 3, title: 'Specific clue', content: 'Sustained baseline + correct samples + tickets stopped.' },
      ...
      successFeedback: 'Sustained baseline, correct samples, tickets stopped — recovery is proven for real users. Now stand down properly.',
      failureFeedback: 'One passing test or a one-minute dip is not proof. Confirm sustained, aggregate user signals before declaring recovery.',
    },
  ],
  reflectionPrompt: 'How many "resolved" incidents re-open because someone trusted a single green check instead of a sustained signal?',
