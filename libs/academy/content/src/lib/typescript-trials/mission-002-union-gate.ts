...
export const ttMission002UnionGate: MissionDefinition = {
  ...
  title: 'Union Gate',
  summary: 'A "status" string accepts any value. Turn it into a checked vocabulary.',
  difficulty: 'easy',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      difficulty: 'easy',
      tags: ['typescript'],
      storyContext: 'Only four statuses are valid, but any string compiles today.',
      prompt: 'Which change makes invalid statuses (and typos) a compile error?',
      options: [
        {
          id: 'a',
          label: "type DeploymentStatus = 'queued' | 'running' | 'succeeded' | 'failed';\n// status: DeploymentStatus",
          isCorrect: true,
          feedback:
        ...
        },
        {
          ...
          isCorrect: false,
          feedback: 'Renaming a loose string is still a loose string — any value, including typos, remains valid.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'You want the compiler to reject any value outside the four.' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        { level: 3, title: 'Specific clue', content: 'The fix lists the four valid statuses as a type.' },
        { level: 4, title: 'Guided solution', content: "Define a DeploymentStatus union of the four literals." },
      ],
      rewards: [{ type: 'xp', amount: 10, label: 'Gate closed' }],
      consequences: [{ type: 'stability', delta: -5, reason: 'A status typo shipped and blanked the deployment badge.' }],
      helpLinks: [{ topicId: 'typescript.union-types', label: 'Union types' }],
      successFeedback: 'The status is now a checked vocabulary — typos fail to compile.',
      failureFeedback: 'Runtime checks and renames leave the string loose. A union type is the compile-time gate.',
    },
  ],
  reflectionPrompt: 'What loose strings in your code would become safer as unions?',
