...
export const ttMission006Refactor: MissionDefinition = {
  ...
  summary: 'Rename a field across the codebase and let the compiler find every caller.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      difficulty: 'medium',
      tags: ['typescript'],
      storyContext: 'Some call sites use customer.name via strongly typed code; a few go through an any-typed helper.',
      ...
      options: [
        ...
        {
          id: 'c',
          label: 'Add firstName/lastName but keep name too, cast where needed, and clean up later',
          isCorrect: false,
          feedback: 'Keeping both fields and casting defers the work and leaves the codebase in a confusing half-migrated state.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'Let the compiler tell you exactly what broke.' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        { level: 3, title: 'Specific clue', content: 'The safe answer changes the type first AND removes the any hiding places.' },
        { level: 4, title: 'Guided solution', content: 'Change the interface, fix each compile error, and drop the any helper.' },
      ],
      rewards: [{ type: 'xp', amount: 25, label: 'Refactored fearlessly' }],
      consequences: [{ type: 'technical-debt', delta: 10, reason: 'A blind find-replace left broken and half-migrated call sites.' }],
      helpLinks: [
        { topicId: 'typescript.interfaces', label: 'Interfaces' },
        { topicId: 'typescript.unknown-vs-any', label: 'unknown vs any' },
      ],
      successFeedback: 'The compiler drove the refactor — every real call site fixed, nothing missed.',
      failureFeedback: 'Text replace and keep-both-and-cast both dodge the type system. Let it enumerate the breakage.',
    },
  ],
  reflectionPrompt: 'How does even one any-typed helper undermine an otherwise type-safe refactor?',
  rewards: [
