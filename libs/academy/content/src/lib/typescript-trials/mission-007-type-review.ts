      ...
      findings: [
        {
          ...
          isCorrect: true,
          feedback: 'A union type would restrict status to real values and catch typos at compile time.',
        },
        {
          id: 'total-any',
          label: 'total is typed any',
          isCorrect: true,
          feedback: 'any on a money field is dangerous — it should be number, checked everywhere it is used.',
        },
        {
          id: 'coupon-required',
          label: 'coupon is required but is sometimes absent',
          isCorrect: true,
          feedback: 'If a coupon can be missing, model it coupon?: { code: string } so the compiler forces a check.',
        },
        {
          id: 'id-string',
          label: 'id should be a number, not a string',
          isCorrect: false,
          feedback: 'String ids are a perfectly valid, common choice — not a weakness.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'Scan for loose strings, any, and mis-marked optionals.' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        { level: 3, title: 'Specific clue', content: 'Three weaknesses: status string, total any, required-but-absent coupon.' },
        { level: 4, title: 'Guided solution', content: 'Flag status, total and coupon. The string id is fine.' },
      ],
      rewards: [{ type: 'xp', amount: 50, label: 'Model reviewed' }],
      consequences: [{ type: 'technical-debt', delta: 15, reason: 'A weak core model spread untyped values through the app.' }],
      helpLinks: [
        { topicId: 'typescript.union-types', label: 'Union types' },
        { topicId: 'typescript.strict-null-checks', label: 'Strict null checks' },
      ],
      successFeedback: 'The core model is now precise — weaknesses caught before they multiplied.',
      failureFeedback: 'Look for the loose string, the any, and the required-but-optional field.',
    },
  ],
  reflectionPrompt: 'Why does a weakness in a shared model cost more than the same weakness in a leaf component?',
