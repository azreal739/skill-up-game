...
export const zodMission005ErrorMapping: MissionDefinition = {
  ...
  campaignId: 'zod-gate',
  title: 'Error Mapping',
  summary: 'The API’s error responses have shapes too. Handle the unhappy path without a second crash.',
  difficulty: 'medium',
  learningObjectives: [
  ...
  challenges: [
    {
      ...
      difficulty: 'medium',
      tags: ['api', 'zod'],
      storyContext: 'Two real problems hide in how this handler treats the error body.',
      prompt: 'Select every genuine issue with the error handling.',
      findings: [
        {
          id: 'assumes-shape',
          label: 'It reads err.error.detail.text but the real body has error.code / error.message',
          isCorrect: true,
          feedback:
        ...
        {
          id: 'no-validation',
          label: 'The error body is used without being validated against a schema',
          isCorrect: true,
          feedback:
        ...
        },
        {
          ...
          isCorrect: false,
          feedback: 'HttpErrorResponse is the correct type for an HttpClient error — that part is right.',
        },
        {
          id: 'sets-message',
          label: 'Assigning to this.message to show the user is a bad pattern',
          isCorrect: false,
          feedback: 'Surfacing a message to the user is exactly what the error path should do.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'Compare the shape the handler reads with the shape the API actually sends.' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        },
        {
          level: 3,
          title: 'Specific clue',
          content: 'The body has error.code and error.message; the handler reaches for error.detail.text.',
        },
        {
          level: 4,
      ...
      consequences: [
        {
          type: 'severity',
          delta: 1,
          reason: 'A crash on the error path masked the original problem and escalated the incident.',
        },
      ],
      helpLinks: [
      ...
      successFeedback: 'The error path is now as trustworthy as the happy path — no second crash.',
      failureFeedback:
    ...
    },
    {
      ...
      options: [
        {
          id: 'a',
          label: '401 → prompt to sign in · 403 → “no access” message · 404 → “customer not found” · 500 → “try again later”',
          isCorrect: true,
          feedback:
        ...
        {
          id: 'c',
          label: '404 → retry automatically · 500 → prompt to sign in · 403 → “not found”',
          isCorrect: false,
          feedback: 'These mappings are scrambled: 500 is not an auth problem and 403 is not a missing resource.',
        },
      ],
      hints: [
        { level: 1, title: 'Direction', content: 'What can the user actually do about each status?' },
        {
          level: 2,
          title: 'Concept',
          content:
        ...
        },
        {
          level: 3,
          title: 'Specific clue',
          content: 'The correct option is the one where 401 leads to sign-in and 500 leads to “try again later”.',
        },
        {
          level: 4,
          title: 'Guided solution',
          content: 'Pick the mapping that treats 401, 403, 404 and 500 as four distinct, sensible outcomes.',
        },
      ],
      ...
      successFeedback: 'Each status now leads the customer to the right next action.',
      failureFeedback: 'A single catch-all message discards what the status code was telling you.',
    },
  ],
  reflectionPrompt: 'Why does the error path deserve the same schema discipline as the success path?',
  rewards: [
